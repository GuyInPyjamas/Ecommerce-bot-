"""
Script to update all UK gift card denominations to the standardized values
"""
import logging
import os
import sys
...
def update_uk_denominations():
    """Update all UK gift card denominations to the standardized values."""
    
    logger.info("Updating all UK gift card denominations to standardized values")
    logger.info("===============================================================")
    
    try:
        with app.app_context():
            # Get UK
            uk = Country.query.filter_by(name="UK").first()
            
            if not uk:
                logger.error("UK not found in the database")
                return
            
            # Get all gift cards for UK
            gift_cards = GiftCard.query.filter_by(country_id=uk.id).all()
            
            if not gift_cards:
                logger.error("No gift cards found for UK")
                return
            
            # The standardized denominations for UK (updated to match requirements)
            ...
            # Commit all changes
            db.session.commit()
            logger.info("All UK gift card denominations have been updated successfully")
    
    except Exception as e:
        logger.error(f"Error updating UK denominations: {e}")
