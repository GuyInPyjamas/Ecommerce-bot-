"""
Script to update all USA gift card denominations to the standardized values
"""
import logging
import os
import sys
...
def update_usa_denominations():
    """Update all USA gift card denominations to the standardized values."""
    
    logger.info("Updating all USA gift card denominations to standardized values")
    logger.info("===============================================================")
    
    try:
        with app.app_context():
            # Get USA
            usa = Country.query.filter_by(name="USA").first()
            
            if not usa:
                logger.error("USA not found in the database")
                return
            
            # Get all gift cards for USA
            gift_cards = GiftCard.query.filter_by(country_id=usa.id).all()
            
            if not gift_cards:
                logger.error("No gift cards found for USA")
                return
            
            # The standardized denominations for USA (updated to match requirements)
            ...
            # Commit all changes
            db.session.commit()
            logger.info("All USA gift card denominations have been updated successfully")
    
    except Exception as e:
        logger.error(f"Error updating USA denominations: {e}")
